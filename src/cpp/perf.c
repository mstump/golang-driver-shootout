...
CassCluster* create_cluster() {
  ...
  cass_cluster_set_queue_size_io(cluster, NUM_CONCURRENT_REQUESTS);
...
void insert_into_perf(CassSession* session, const char* query, const CassPrepared* prepared) {
  ...
  for (i = 0; i < NUM_CONCURRENT_REQUESTS; ++i) {
    ...
    if (rc != CASS_OK && rc != CASS_ERROR_LIB_NO_HOSTS_AVAILABLE && rc != CASS_ERROR_LIB_REQUEST_QUEUE_FULL) {
      print_error(future);
    }
    cass_future_free(future);
  }
...
void run_insert_queries(void* data) {
  ...
  const char* insert_query = "INSERT INTO songs (id, title, album, artist) VALUES (?, ?, ?, ?);";
...
int main() {
  ...
  while (status_wait(&status, 5) > 0) {
    ...
    printf("rate stats (requests/second): mean %f 1m %f 5m %f 10m %f connection_timeouts %llu pending_request_timeouts %llu request_timeouts %llu\n",
